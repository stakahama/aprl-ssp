...
class searchgroups:
    ...
    def matchatoms(self,smilesstr):
        ...
        ##
        allatomidx = set(reduce(add,reduce(set.union,pairs.values())))
        atomicmass = [(atom.type,atom.atomicmass) for atom in mol.atoms if atom.idx in allatomidx]
        self.atomicmass += atomicmass
        extract = lambda dictionary,values: [map(dictionary.get,v) for v in values]
        atomtype = {atom.idx:(atom.idx,atom.type) for atom in mol.atoms}
        usepairs = OrderedDict([(k,extract(atomtype,pairs[k])) for k in pairs if include.ix[k]])
        return pd.Series(usepairs,dtype=object)
    ##
...
if __name__=='__main__':
    ...
    ## --- experimental (is not sensible) ---

    ## create 3-D array

    atomtype = map(itemgetter(0),atomicmass)

    def count(atomtype): ## this double/triple counts the same atoms so not correct
        def fn(x):
            atoms = map(itemgetter(1),map(itemgetter(0),x))
            return map(atoms.count,atomtype)
        return fn

    att = map(count(atomtype),output.values.reshape(-1))

    arr = np.array(att,dtype=int).reshape(output.shape+(len(atomtype),))

    axes = [pd.Series(range(len(output.index)),index=output.index.values),
            pd.Series(range(len(output.columns)),index=output.columns.values),
            pd.Series(range(len(atoms)),index=atoms)]
